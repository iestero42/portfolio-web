...
const MatrixRainingCode = () => {
  ...
  useEffect(() => {
	...
	const colorBackground = `rgba(${hexToRgb(getComputedStyle(document.documentElement).getPropertyValue('--color-60'))}, 0.2)`;
    ...
    let columns = Math.floor(width / 15); // Number of columns based on character width
    ...
    const draw = () => {
      ...
      ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--color-10'); // Green color for characters
